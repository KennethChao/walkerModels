...
%% Solver setup
% ode45 solver option
optionsStance = odeset('Event', @liftOffEventFcn, 'AbsTol', 1e-9, 'RelTol', 1.e-8); %,
dymStance = @(t, x) dymModelStanceSLIPPendulum(t, x, u0, parms); %dymModelStanceDimensionless
optionsFlight = odeset('Event', @(t, x)touchDownSLIPPendulumEventFcn(t, x, parms), 'AbsTol', 1e-9, 'RelTol', 1.e-8); %,

...
function result = oneStepPerturbedSimulationSLIP(delta0,perturbationSet, parms)
...
elseif strcmp(mode, 'perturbedSimulation')
    perturbation = 1e-4;
    iterNumb = 2;
elseif strcmp(mode, 'robustnessTest')
    iterNumb= 200;
end
...
for i = 1:iterNumb
    ...
    elseif strcmp(mode, 'robustnessTest')
        optionsFlight = odeset('Event', @(t, x)eventFcnPerturbedTouchDownSLIP(t, x, perturbationSet(i), beta), 'RelTol', 1.e-8);
    end
